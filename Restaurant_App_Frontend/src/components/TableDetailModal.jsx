...
function TableDetailModal({ table, activeOrder, onClose, onUpdate }) {
    ...
    const loadData = async () => {
        try {
            // Sipariş detaylarını ve ürün listesini (ekleme yapmak için) çek
    ...
    const handleRemoveItem = async (itemId) => {
        if (!window.confirm("Bu ürünü adisyondan silmek istiyor musunuz?")) return;
        try {
    ...
    const handleCloseAccount = async () => {
        if (!window.confirm(`Toplam ${orderDetails?.totalAmount} ₺ tahsil edildi mi? Masa kapatılacak.`)) return;
        try {
                ...
                <div className="modal-content">
                    <div className="modal-header bg-dark text-white">
                        <h5 className="modal-title">
                            ...
                            Masa {table.tableNumber} Adisyonu
                        </h5>
                    ...
                    <div className="modal-body">
                        ...
                        <div className="table-responsive mb-4">
                            <table className="table table-hover align-middle">
                                ...
                                <tbody>
                                    {orderDetails?.orderItemsInRestaurant?.map(item => (
                                        <tr key={item.id}>
                                            ...
                                            <td>
                                                <button className="btn btn-sm btn-outline-danger border-0" onClick={() => handleRemoveItem(item.id)}>
                                ...
                                <tfoot className="table-group-divider">
                                    <tr>
                                        ...
                                        <td className="text-end fw-bold fs-5 text-success">{orderDetails?.totalAmount} ₺</td>
                                        <td></td>
                                    </tr>
                                </tfoot>
                            </table>
                        ...
                        {/* ÜRÜN EKLEME PANELİ */}
                        {showAddProduct ? (
                            <div className="card bg-light border-0 p-3 mb-3 fade-in-up">
                                ...
                                <div className="d-flex gap-2">
                                    ...
                                    />
                                    <button className="btn btn-success text-nowrap" onClick={handleAddItem}>Ekle</button>
                                    <button className="btn btn-outline-secondary" onClick={() => setShowAddProduct(false)}>İptal</button>
                                </div>
                            </div>
                        ) : (
                            <button className="btn btn-outline-dark w-100 mb-3 dashed-border" onClick={() => setShowAddProduct(true)}>
                                <i className="fas fa-plus me-2"></i> Sipariş Ekle
                            </button>
                        )}
                    </div>
                    <div className="modal-footer justify-content-between">
                        <div className="text-muted small">
                            ...
                            Sipariş: #{activeOrder.id} | {new Date(activeOrder.orderDate).toLocaleTimeString()}
                        </div>
                        <button className="btn btn-success px-4" onClick={handleCloseAccount}>
                            <i className="fas fa-check-double me-2"></i> Hesabı Kapat
                        </button>
                    </div>
                </div>
            </div>
