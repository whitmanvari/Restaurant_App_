...
function MenuPage() {
    ...
    useEffect(() => {
        const params = {
            ...
            pageSize: 9, // Grid için 9 daha uygun
            searchTerm,
    ...
    return (
        <div className="menu-page-wrapper" style={{ backgroundColor: '#f9f9f9', minHeight: '100vh', paddingTop: '100px', paddingBottom: '60px' }}>
            <div className="container">
                ...
                <div className="row g-5">
                    ...
                    <div className="col-lg-3">
                        <div className="filter-sidebar bg-white p-4 rounded shadow-sm sticky-top" style={{ top: '100px', zIndex: 900 }}>
                            ...
                            <div className="mb-4">
                                ...
                                <ul className="list-group list-group-flush">
                                    <li 
                                        className={`list-group-item list-group-item-action d-flex justify-content-between align-items-center cursor-pointer ${activeCategory === 'Tümü' ? 'active-cat' : ''}`}
                                        onClick={() => handleCategoryClick('Tümü')}
                                        style={{ cursor: 'pointer', border: 'none', padding: '10px 0', fontWeight: activeCategory === 'Tümü' ? 'bold' : 'normal', color: activeCategory === 'Tümü' ? '#c5a059' : '#555' }}
                                    >
                                        <span>Tümü</span>
                                    ...
                                    {categories.map(cat => (
                                        <li 
                                            ...
                                            style={{ cursor: 'pointer', border: 'none', padding: '10px 0', fontWeight: activeCategory === cat.name ? 'bold' : 'normal', color: activeCategory === cat.name ? '#c5a059' : '#555' }}
                                        >
                                            <span>{cat.name}</span>
                            ...
                            {/* Alerjenler */}
                            <div className="mb-3">
                                <label className="form-label fw-bold small text-uppercase ls-1 text-danger">Alerjen Filtresi (İçermesin)</label>
                                <div className="d-flex flex-wrap gap-2">
                                    {Allergens.map(allergen => {
                                        ...
                                        return (
                                            <button 
                                                key={allergen.id}
                                                className={`btn btn-sm ${isSelected ? 'btn-danger text-white' : 'btn-outline-light text-dark border'}`}
                                                onClick={() => handleAllergenChange(allergen.id)}
                                                title={`${allergen.label} içermeyenleri göster`}
                                        ...
                                        <nav>
                                            <ul className="pagination">
                                                <li className={`page-item ${pagination.pageNumber === 1 ? 'disabled' : ''}`}>
                                                    <button className="page-link border-0 bg-transparent text-dark" onClick={() => dispatch(setPageNumber(pagination.pageNumber - 1))}>
                                                ...
                                                {[...Array(pagination.totalPages)].map((_, i) => (
                                                    <li key={i} className="page-item">
                                                        <button 
                                                            className={`page-link border-0 rounded-circle mx-1 ${pagination.pageNumber === i + 1 ? 'bg-dark text-white' : 'bg-white text-dark'}`}
