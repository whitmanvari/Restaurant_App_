...
function ReservationPage() {
    ...
    useEffect(() => {
        if (formData.reservationDate) checkAvailability();
    ...
    const checkAvailability = async () => {
        ...
        try {
            ...
            console.error(err);
            toast.error("Masa sorgulama sırasında bir hata oluştu.");
        } finally {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            toast.success("Rezervasyon talebiniz başarıyla gönderildi. Admin onayı bekleniyor.");

            // Formu sıfırla ve listeyi yenile
            setFormData({ reservationDate: '', numberOfGuests: 2, specialRequests: '' });
            setSelectedTableId('');
            setTables([]);
            checkAvailability(); // Yeni durumda masayı bloke görmeli
        } catch (error) {
    ...
    return (
        <div className="d-flex flex-wrap" style={{ minHeight: '100vh', paddingTop: '80px' }}>
            ...
            {/* SAĞ FORM */}
            <div className="col-lg-6 d-flex align-items-center justify-content-center p-5" style={{ backgroundColor: 'var(--bg-card)', color: 'var(--text-main)' }}>
                <div style={{ maxWidth: '500px', width: '100%' }}>
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="mb-4">
                            <label className="form-label fw-bold small text-uppercase">Masa Tercihi {loadingTables && <span className="spinner-border spinner-border-sm ms-2 text-warning"></span>}</label>
                            {!formData.reservationDate ? (
                                ...
                                >
                                    ...
                                    {tables.length === 0 && !loadingTables && <option disabled>Seçilen saatte uygun masa yok.</option>}
                                </select>
                            )}
                        </div>
